...
void DistanceSensorArrayROS::distancesChangedEvent(const float* distances, unsigned int size)
{
	...
	for(unsigned int i = 0; i < size; ++i)
	{
		...
		// 0.2 is the radius of the robot
                // Cogemos la distancia desde el propio sensor y como la da en metros multiplicamos por 100 para obetener los centimetros
                distances_msg_.points[i].x = distances[i] * 100;  
		distances_msg_.points[i].y = distances[i] * 100;
